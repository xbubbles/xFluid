...
bool TriangleMesh::loadBOBJ(std::string BOBJFilename) {
	...
	std::vector<Vec3d> vertices(numverts);
	if (numverts > 0) {
...
void TriangleMesh::updateVertexNormals() {
	...
	for (unsigned int i = 0; i < _vertexTriangles.size(); i++) {
		...
		for (unsigned int j = 0; j < _vertexTriangles[i].size(); j++) {
			n = n + facenormals[_vertexTriangles[i][j]];
		}

		n = (n / (float)_vertexTriangles[i].size()).normalize();
		normals.push_back(n);
	}
}
...
double TriangleMesh::getTriangleArea(int tidx) {
	...
	Vec3d AB = vertices[t.tri[1]] - vertices[t.tri[0]];
	Vec3d AC = vertices[t.tri[2]] - vertices[t.tri[0]];
...
bool TriangleMesh::_loadPLYVertexData(std::ifstream *file, std::string &header) {
	...
	if (isColorEnabled) {
		...
		for (int i = 0; i < numVertices; i++) {
			...
			vertexcolors.push_back(Vec3d(r / 255.0, g / 255.0, b / 255.0));
		}
	}
	else {
		vertices.assign((Vec3d*)vertexData, (Vec3d*)vertexData + numVertices);
	}
	delete[] vertexData;
...
void TriangleMesh::getTrianglePosition(unsigned int index, Vec3d tri[3]) {
...
Vec3d TriangleMesh::getTriangleNormal(unsigned int index) {
	//FLUIDSIM_ASSERT(index < triangles.size());
	...
	return (normals[t.tri[0]] + normals[t.tri[1]] + normals[t.tri[2]]).normalize();
}

Vec3d TriangleMesh::getBarycentricCoordinates(unsigned int index, Vec3d p) {
	Triangle t = triangles[index];
	int size = (int)vertices.size();
	//FLUIDSIM_ASSERT(t.tri[0] < size && t.tri[1] < size && t.tri[2] < size);

	Vec3d a = vertices[t.tri[0]];
	Vec3d b = vertices[t.tri[1]];
	Vec3d c = vertices[t.tri[2]];
	Vec3d normal = getTriangleNormal(index);

	float areaABC = normal * (b - a).cross(c - a);
	float areaPBC = normal * (b - p).cross(c - p);
	float areaPCA = normal * (c - p).cross(a - p);

	float bx = areaPBC / areaABC;
	float by = areaPCA / areaABC;
	float bz = 1.0f - bx - by;

	return Vec3d(bx, by, bz);
}

Vec3d TriangleMesh::getTriangleNormalSmooth(unsigned int index, Vec3d p) {
	//FLUIDSIM_ASSERT(index < triangles.size());
	...
	Vec3d bary = getBarycentricCoordinates(index, p);

	return normals[t.tri[0]]* bary.x + normals[t.tri[1]]* bary.y + normals[t.tri[2]]* bary.z;
}

Vec3d TriangleMesh::getTriangleFaceDirection(unsigned int index) {
	//FLUIDSIM_ASSERT(index < triangles.size());
...
Vec3d TriangleMesh::getTriangleCenter(unsigned int index) {
	//FLUIDSIM_ASSERT(index < triangles.size());
...
void TriangleMesh::_smoothTriangleMesh(double value, std::vector<bool> &isSmooth) {
	...
	for (unsigned int i = 0; i < vertices.size(); i++) {
		...
		for (unsigned int j = 0; j < _vertexTriangles[i].size(); j++) {
			...
			if (t.tri[0] != (int)i) {
				avg = avg + vertices[t.tri[0]];
				count++;
			}
			if (t.tri[1] != (int)i) {
				avg = avg + vertices[t.tri[1]];
				count++;
			}
			if (t.tri[2] != (int)i) {
				avg =avg + vertices[t.tri[2]];
				count++;
			}
		}
...
double TriangleMesh::_getSignedTriangleVolume(unsigned int tidx) {
	Vec3d p1 = vertices[triangles[tidx].tri[0]];
	Vec3d p2 = vertices[triangles[tidx].tri[1]];
	Vec3d p3 = vertices[triangles[tidx].tri[2]];
...
void TriangleMesh::removeExtraneousVertices() {
	...
	std::vector<Vec3d> newVertexList;
	std::vector<Vec3d> newVertexColorList;
...
bool TriangleMesh::_isPolyhedronHole(std::vector<int> &poly) {
	...
	for (unsigned int i = 0; i < poly.size(); i++) {
		centroid = centroid + getTriangleCenter(poly[i]);
	}
	centroid = centroid / poly.size();
...
void TriangleMesh::translate(Vec3d trans) {
	for (unsigned int i = 0; i < vertices.size(); i++) {
...
AABB TriangleMesh::_getMeshVertexIntersectionAABB(std::vector<Vec3d> verts1,
	...
	for (unsigned int i = 0; i < vertices.size(); i++) {
		...
		if (query.size() == 1) {
			...
			for (unsigned int idx = 0; idx < query.size(); idx++) {
				Vec3d v = vertices[i] - vertices[query[idx].id];
				double distsq = v.norm();
				if (distsq < mindist) {
					mindist = distsq;
...
void TriangleMesh::_findDuplicateVertexPairs(std::vector<int> &verts1,
	...
	for (unsigned int i = 0; i < verts1.size(); i++) {
		...
		Vec3d v1 = vertices[verts1[i]] - offset;
		query.clear();
